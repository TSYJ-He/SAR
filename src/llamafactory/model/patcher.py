...
def patch_model_for_sar(model: "PreTrainedModel", finetuning_args: "FinetuningArguments") -> None:
    if not finetuning_args.use_sar:
        return
    logger.info_rank0("Applying SAR patch to the model.")
    # （1 Patch Vision Encoder 以捕获中间层 hidden_states
    if hasattr(model, "vision_tower") and hasattr(model.vision_tower, "vision_tower"):
        vision_encoder = model.vision_tower.vision_tower
    elif hasattr(model, "model") and hasattr(model.model, "visual"):  # for LLaVA-OneVision
        vision_encoder = model.model.visual
    else:
        logger.warning_rank0("Could not find vision tower to patch for SAR. Skipping.")
        return
    original_vision_forward = vision_encoder.forward
    # 使用Hook来捕获中间layer的feature
    def patched_vision_forward(self, *args, **kwargs):
        model._sar_vision_hidden_states = None  # 重置状态
        ...
        target_layer_idx = (len(layers) * 2) // 3
        def hook_fn(module, input, output):
            # output可能是元组，也可能是tensor 对吧
            if isinstance(output, tuple):
                model._sar_vision_hidden_states = output[0].detach()
            else:
                model._sar_vision_hidden_states = output.detach()
        ...
        if model._sar_vision_hidden_states is None:
    ...
    #  2) Patch Decoder Attention
    if hasattr(model, "model") and hasattr(model.model, "layers"):
        decoder_layers = model.model.layers
    elif hasattr(model, "model") and hasattr(model.model, "language_model") and hasattr(model.model.language_model,
                                                                                        "layers"):
        decoder_layers = model.model.language_model.layers
        ...
        logger.warning_rank0("Could not find decoder layers to patch for SAR. Skipping.")
        return
    # 为Attention模块创建一个新的forward方法来实现presoftmax
    def make_sar_forward(sar_inst, orig_forward_fn):
        # 捕获原始函数的签名以支持不同的attention实现 (FlashAttention啥的)
        original_signature = inspect.signature(orig_forward_fn)

        def sar_forward(self, hidden_states: torch.Tensor, *args, **kwargs):
            # 将args和kwargs绑定到签名上，方便访问
            bound_args = original_signature.bind(self, hidden_states, *args, **kwargs)
            bound_args.apply_defaults()

            output_attentions = bound_args.arguments.get("output_attentions", False)

            # 重新实现attention计算pre-softmax scores
            ...
            # RoPE
            past_key_value = bound_args.arguments.get("past_key_value")
            ...
            if position_ids is None:
                position_ids = torch.arange(kv_seq_len - q_len, kv_seq_len, dtype=torch.long,
                                            device=hidden_states.device).unsqueeze(0)

            cos, sin = self.rotary_emb(value_states, seq_len=kv_seq_len)
            query_states, key_states = transformers.models.llama.modeling_llama.apply_rotary_pos_emb(query_states,
                                                                                              key_states, cos,
                                                                                                     sin, position_ids)
            ...
            # **关键步骤**: 计算 pre-softmax scores
            attn_scores = torch.matmul(query_states, key_states.transpose(2, 3)) / torch.sqrt(
                torch.tensor(self.head_dim, dtype=torch.float32))

            attention_mask = bound_args.arguments.get("attention_mask")
            if attention_mask is not None:
                if attention_mask.size() != (bsz, 1, q_len, kv_seq_len):
                    attention_mask = transformers.modeling_attn_mask_utils._prepare_4d_causal_attention_mask_for_sdpa(
                        attention_mask, (bsz, q_len), key_states, 0
                    )
                attn_scores = attn_scores + attention_mask

            # 计算 post-softmax probabilities
            attn_weights = torch.nn.functional.softmax(attn_scores, dim=-1, dtype=torch.float32).to(query_states.dtype)
            ...
            if sar_inst.is_active and not self.training:
                ...
                if input_ids is not None:
                    head_weights = sar_inst.run(
                        pre_softmax_scores=attn_scores.detach(),
                        attention_probs=attn_weights.detach(),
                        input_ids=input_ids,
                        vision_encoder_hidden_states=getattr(model, '_sar_vision_hidden_states', None),
                    )

                    if head_weights is not None:
                        # attention_output已经是 per-head 的形式: (bsz, num_heads, q_len, head_dim)
                        reweighted_output = sar_inst.apply_reweighting(attn_output, head_weights)
                        attn_output = reweighted_output
    ...
    def generate_with_sar_context(self, *args, **kwargs):
        ...
        try:
            ...
            # 清理上下文
            if hasattr(model, "_current_input_ids"):
                del model._current_input_ids
            if hasattr(model, "_sar_vision_hidden_states"):
                del model._sar_vision_hidden_states

    model.generate = MethodType(generate_with_sar_context, model)
    logger.info_rank0("Patched model.generate to provide context for SAR.")
