...
class FinetuningArguments(
    ...
    visual_contrastive_loss_weight: float = field(
    default=0.0,
    metadata={"help": "The weight for the auxiliary visual contrastive loss used in the control group experiment."},
    )

    use_sar: bool = field(
        default=False,
        metadata={"help": "Whether or not to use Semantic Attention Re-weighting (SAR) during inference."},
    )
    sar_activation_layer_k: int = field(
        default=10,
        metadata={"help": "The number of final decoder layers (K) to apply SAR."},
    )
    sar_beta: float = field(
        default=0.5,
        metadata={"help": "The temperature parameter (beta) for SAR's soft re-weighting softmax."},
    )
